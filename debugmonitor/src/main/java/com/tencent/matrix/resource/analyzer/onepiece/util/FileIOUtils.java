...
public final class FileIOUtils {
    ...
    public static boolean writeFileFromIS(File file, final InputStream is, boolean append) {
        try {
            return writeFileFromISThrowsException(file, is, append);
        } catch (Exception e) {
            e.printStackTrace();
            return false;
        }
     ...
     * @return {@code true}: 写入成功<br>{@code false}: 写入失败
     */
    public static boolean writeFileFromISThrowsException(File file, final InputStream is, boolean append) throws
            Exception {
        if (!FileUtils.createOrExistsFile(file) || is == null) {
            throw new IOException("createOrExistsFile " + file + " failed, or " + is + " is null");
        }
     ...
     * @return {@code true}: 写入成功<br>{@code false}: 写入失败
     */
    public static boolean writeFileFromBytesByChannel(String filePath, final byte[] bytes, boolean append, boolean
            isForce) {
     ...
     * @return {@code true}: 写入成功<br>{@code false}: 写入失败
     */
    public static boolean writeFileFromBytesByMap(String filePath, final byte[] bytes, boolean append, boolean
            isForce) {
