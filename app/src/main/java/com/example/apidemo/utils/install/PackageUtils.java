package com.example.apidemo.utils.install;
...
import com.example.apidemo.utils.install.ShellUtils.CommandResult;
 ...
 *
 * 弹出安装页面安装 or 静默安装
 * root权限静默安装实际使用的是su pm install -r filePath命令。
 * 如：PackageUtils.installSilent(context, Environment.getExternalStorageDirectory().getAbsolutePath() + "/apk/dev-tools.apk");
 */
