...
public class DetecterActivity extends Activity implements OnCameraListener, View.OnTouchListener, Camera.AutoFocusCallback, View.OnClickListener {
	...
	Runnable noFaceDetect = new Runnable() {
		@Override
		public void run() {
			mTextView.setText("检测不到人脸");
	...
	class FRAbsLoop extends AbsLoop {
		...
		List<FaceDB.FaceRegist> mResgist = ArcFaceWrapper.getInstance(DetecterActivity.this).mFaceDB.mRegister;
		// ((ArcFaceWrapper)DetecterActivity.this.getApplicationContext())
		...
		public void loop() {
			if (mImageNV21 != null) {
				...
				if (max > 0.6f) {	//	匹配成功！！！tudo。。。。。。。。。。。。。。。。。。
					//fr success.

					Log.w(TAG, "fit Score:" + max + ", NAME:" + name);
					final String mNameShow = name;
					mHandler.removeCallbacks(noFaceDetect);
					...
					DetecterActivity.this.runOnUiThread(new Runnable() {
						...
						public void run() {
							mTextView.setAlpha(1.0f);
							mTextView1.setVisibility(View.VISIBLE);
							mTextView1.setText( gender + "," + age + "置信度：" + (float)((int)(max_score * 1000)) / 1000.0);
							mTextView1.setTextColor(Color.RED);
							mTextView.setText(mNameShow);
							mTextView.setTextColor(Color.RED);
							mImageView.setImageAlpha(255);
	...
	public Object onPreview(byte[] data, int width, int height, int format, long timestamp) {
		...
		Log.v(TAG, "mCameraRotate =" + mCameraRotate + "AFT_FSDK_FaceFeatureDetect =" + err.getCode());
		Log.v(TAG, "Face size=" + result.size());
		...
		if (mImageNV21 == null) {
			if (!result.isEmpty()) {
				...
				if (!isPostted) {
					mHandler.removeCallbacks(noFaceDetect);
					mHandler.postDelayed(noFaceDetect, 2000);
					isPostted = true;
				}
			}
		}
	...
	public void onClick(View view) {
		if (view.getId() == R.id.imageButton) {
			...
			mGLSurfaceView.setRenderConfig(mCameraRotate, mCameraMirror);	// 加上此句，更改同步到mGLSurfaceView中
